...
- 公网访问：`https://ttyd.hbyrkj.top`
- 公网入口：阿里云服务器上的 Nginx
- 转发链路：`Nginx -> frps(7080) -> frpc -> 本机 ttyd:7681`
- 认证方式：浏览器打开页面时弹出用户名密码框

## 2. 本机环境信息

- ttyd 二进制：`/home/yerong/ProgramFiles/ttyd/ttyd`
- ttyd 源码目录：`/home/yerong/ProgramFiles/ttyd-src`
- ttyd 服务文件：`/home/yerong/.config/systemd/user/ttyd.service`
- ttyd 凭据文件：`/home/yerong/.config/ttyd/ttyd.env`
- frpc 配置：`/home/yerong/ProgramFiles/Frp/frpc.toml`
- 域名：`ttyd.hbyrkj.top`
- 本机局域网 IP：`192.168.110.218`
...
```bash
mkdir -p /home/yerong/ProgramFiles/ttyd
cd /home/yerong/ProgramFiles/ttyd
...
```bash
git clone --branch 1.7.7 --depth 1 https://github.com/tsl0922/ttyd.git /home/yerong/ProgramFiles/ttyd-src
```

### 4.2 修改源码补丁

编辑 `/home/yerong/ProgramFiles/ttyd-src/src/protocol.c`，让 `/ws` 握手不再强依赖 Basic Auth header。
...
- `/home/yerong/ProgramFiles/ttyd-src/html/package.json`
- `/home/yerong/ProgramFiles/ttyd-src/html/src/components/terminal/xterm/index.ts`
...
- 这些属于常规 C 项目编译环境
- 构建完成后可以删除源码目录里的构建中间目录，不影响已经部署好的 `/home/yerong/ProgramFiles/ttyd/ttyd`

### 4.4 重新构建前端并生成新的 `html.h`

在 `/home/yerong/ProgramFiles/ttyd-src/html` 下执行：
...
- 安装前端依赖
- 重新打包浏览器端资源
- 通过 `gulp` 生成新的 `/home/yerong/ProgramFiles/ttyd-src/src/html.h`
...
```bash
mkdir -p /home/yerong/.config/ttyd
chmod 700 /home/yerong/.config/ttyd
cat > /home/yerong/.config/ttyd/ttyd.env <<'EOF'
TTYD_CREDENTIAL=your_username:your_password
EOF
chmod 600 /home/yerong/.config/ttyd/ttyd.env
```
...
创建 `/home/yerong/.config/systemd/user/ttyd.service`：
...
[Service]
Type=simple
WorkingDirectory=/home/yerong/ProgramFiles
Environment=TERM=xterm-256color
EnvironmentFile=/home/yerong/.config/ttyd/ttyd.env
ExecStart=/bin/sh -lc 'exec /home/yerong/ProgramFiles/ttyd/ttyd --interface 0.0.0.0 --port 7681 --check-origin -c "$TTYD_CREDENTIAL" --writable /bin/bash -l'
Restart=on-failure
RestartSec=2
...
```bash
sudo loginctl enable-linger yerong
```
...
编辑 `/home/yerong/ProgramFiles/Frp/frpc.toml`，加入：
...
```bash
/home/yerong/ProgramFiles/Frp/frpc verify -c /home/yerong/ProgramFiles/Frp/frpc.toml
...
- `/etc/letsencrypt/live/ttyd.hbyrkj.top/fullchain.pem`
- `/etc/letsencrypt/live/ttyd.hbyrkj.top/privkey.pem`

创建或修改 `ttyd.hbyrkj.top` 的 Nginx 配置：

```nginx
server {
    listen 80;
    server_name ttyd.hbyrkj.top;
...
server {
    ...
    ssl_certificate     /etc/letsencrypt/live/ttyd.hbyrkj.top/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/ttyd.hbyrkj.top/privkey.pem;
...
```bash
curl -kI -u 'your_username:your_password' https://ttyd.hbyrkj.top
```
...
```bash
curl -sku 'your_username:your_password' https://ttyd.hbyrkj.top/token
```
...
curl --http1.1 -isk --max-time 15 \
  ...
  -H 'Sec-WebSocket-Version: 13' \
  -H 'Sec-WebSocket-Key: SGVsbG8sIHdvcmxkIQ==' \
  -H 'Origin: https://ttyd.hbyrkj.top' \
  https://ttyd.hbyrkj.top/ws
```
...
1. 打开 `https://ttyd.hbyrkj.top`
2. 浏览器弹出用户名密码框
3. 输入 `/home/yerong/.config/ttyd/ttyd.env` 中的账号密码
4. 认证通过后进入终端页面
...
- `ttyd` 已安装在 `/home/yerong/ProgramFiles/ttyd`
- 当前运行的是从 `/home/yerong/ProgramFiles/ttyd-src` 编译出来的补丁版静态二进制
- 当前前端已经启用 xterm `ClipboardAddon`，用于支持 `opencode` 等 TUI 的浏览器剪贴板同步
- `ttyd` 用户级 systemd 服务已创建并启用
- `ttyd` 监听在 `0.0.0.0:7681`
- `frpc` 已包含 `ttyd.hbyrkj.top` 的 HTTP 代理
- `ttyd.hbyrkj.top` 证书已签发
